/*
  Feinstaubmessgerät zur Messung der Feinstaubkonzentration in der Umgebungsluft.
  Pollution measurement unit to determine the concentration of PM10 and PM2.5 particulate matter in the outside air.
...
void setup() {
  ...
  // Connecting to Wifi-Hotspot and start web-server.
  ...
  // Show IP address on the display and wait for the connection of a device.
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("IP-Adresse:");
  lcd.setCursor(0, 1);
  lcd.print(WiFi.localIP());
  // To increase the lifetime of the sensor.
  sds.sleep();
}

void loop() {
  server.handleClient();  // wait for clients.
  // OPTIONAL: Comment out this code for a cyclic measurement.
  //refreshValues();
  //delay(300000);
